...
const EditDependencia = ({ pathname, query, success, dependencia }) => {
  ...
  const [form, setForm] = useState(
    JSON.parse(JSON.stringify(dependencia || {}))
  );
  ...
  const save = async () => {
    let answer = await Confirm(
      "warning",
      `¿Estas seguro en guardar los datos?`,
      "Estoy seguro"
    );
    if (!answer) return false;
    setCurrentLoading(true);
    await authentication
      .post(`dependencia/${form.id}?_method=PUT`, form)
      .then(async (res) => {
              ...
              <div className="col-md-9">
                <div className="row justify-content-end">
                  ...
                  <div className="col-md-6 mb-3">
                    <Form.Field
                      error={
                        errors && errors.descripcion && errors.descripcion[0]
                          ? true
                          : false
                      }
                    >
                      <label htmlFor="">
                        Descripción <b className="text-red">*</b>
                      </label>
                      <input
                        type="text"
                        name="descripcion"
                  ...
                  <div className="col-md-6 mb-3">
                    <Form.Field
                      error={
                        errors && errors.ubicacion && errors.ubicacion[0]
                          ? true
                          : false
                      }
                    >
                      <label htmlFor="">
                        Ubicación <b className="text-red">*</b>
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="col-md-6 mb-3">
                    <Form.Field
                      error={
                        errors && errors.type && errors.type[0] ? true : false
                      }
                    >
                      <label htmlFor="">
                        Tipo <b className="text-red">*</b>
                      </label>
                      <Select
...
EditDependencia.getInitialProps = async (ctx) => {
  ...
  let { success, dependencia } = await authentication
    .get(`dependencia/${id}`, {}, ctx)
    .then((res) => res.data)
