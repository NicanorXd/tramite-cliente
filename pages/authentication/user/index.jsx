...
const UserIndex = ({ pathname, query, success, users }) => {
  ...
  const setChangeState = async (obj, condicion) => {
    let answer = await Confirm(
      "warning",
      `¿Deseas ${condicion ? "restaurar" : "desactivar"} la Cuenta ${
        obj.email
      }?`
    );
    ...
    await authentication
      .post(`user/${obj.id}/state?_method=PUT`, { state: condicion })
      .then(async (res) => {
...
UserIndex.getInitialProps = async (ctx) => {
  ...
  // request
  let { success, users } = await authentication
    .get(`user?${query_string}`, {}, ctx)
    .then((res) => res.data)
    .catch((err) => {
