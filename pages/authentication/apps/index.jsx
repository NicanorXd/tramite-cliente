...
const AppIndex = ({ pathname, query, apps }) => {
  ...
  const changeState = async (obj, condicion = 0) => {
    let answer = await Confirm(
      "warning",
      `¿Desea ${condicion ? "restaurar" : "desactivar"} al aplicación "${
        obj.name
      }"?`,
      `${condicion ? "Restaurar" : "Desactivar"}`
    );
    if (!answer) return false;
    app_context.setCurrentLoading(true);
    await authentication
      .post(`app/${obj.id}/state?_method=PUT`, { state: condicion })
      .then(async (res) => {
      ...
      >
        <Datatable
          ...
          options={[
            ...
            {
              key: "block",
              icon: "fas fa-b/authentication/useran",
              title: "Restricciones",
            },
            {
              key: "delete",
...
AppIndex.getInitialProps = async (ctx) => {
  ...
  query.query_search =
    typeof query.query_search == "undefined" ? "" : query.query_search;
  let query_string = `page=${query.page}&query_search=${query.query_search}`;
  // listar apps
  let { success, apps } = await authentication
    .get(`app?${query_string}`, {}, ctx)
    .then((res) => res.data)
