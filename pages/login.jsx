...
const Login = ({ pathname, query }) => {
  ...
  if (option == "VERIFICATION")
    return <VerificarAuthentication my_app={app_context.app || {}} />;
      ...
      <header id="auth-header" className={`mt-5 text-center`}>
        <a href={app_context.app && app_context.app.support}>
          <img
            src={
              app_context.app &&
              app_context.app.icon_images &&
              app_context.app.icon_images.icon_200x200
            }
      ...
      <form className="auth-form" onSubmit={(e) => e.preventDefault()}>
        ...
        >
          <div className="form-label-group">
            ...
            >
              <i
                className={`fas fa-${show_password ? "eye" : "eye-slash"}`}
              ></i>
            </span>
          </div>
          <label className="text-danger">
        ...
        <div className="text-center pt-0">
          <Link href="/recovery_password">
            <a className="link">Recuperar cuenta</a>
          </Link>
        </div>
      </form>
