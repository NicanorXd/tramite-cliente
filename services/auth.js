...
export const VERIFY = async (ctx, systemSlug, name = "", type = "MODULE") => {
  ...
  console.log(`auth/permission/${systemSlug}/${type}?name=${name}`);
  let { allow } = await authentication
    .get(`auth/permission/${systemSlug}/${type}?name=${name}`, {}, ctx)
    .then((res) => res.data)
    .catch((err) => {
      console.log(err);
      return { success: false, status: err.status || 501, allow: false };
    });
  //  validar permiso
