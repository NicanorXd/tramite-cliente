...
const ShowInfo = ({ validateFile = [], onArchived = null }) => {
  ...
  // tramite
  const { current_tracking, dependencia_id, dispatch, setOption, setFile } =
    useContext(TramiteContext);
  ...
  const verifyTracking = async () => {
    let answer = await Confirm(
      `warning`,
      `¿Estás seguro en ${
        current_tracking.revisado ? "quitar la autorización" : "autorizar"
      } el trámite?`,
      "Autorizar"
    );
    ...
    await trackingProvider
      .verify(
        current_tracking.id,
        { revisado: current_tracking.revisado ? 0 : 1 },
        options
      )
      .then(async (res) => {
  ...
  const handleFile = (action, currentFile, file) => {
    if (action == "delete") {
      dispatch({
        type: tramiteTypes.DELETE_FILE_TRACKING,
        payload: currentFile,
      });
    } else {
  ...
  return (
    <>
      <div className="col-md-10 mb-2">
        <CardInfoTramite
          image={
            (current_tramite.person &&
              current_tramite.person.image_images &&
              current_tramite.person.image_images.image_200x200) ||
            ""
          }
          remitente={
            (current_tramite.person && current_tramite.person.fullname) || ""
          }
          email={
            (current_tramite.person && current_tramite.person.email_contact) ||
            ""
          }
          dependencia={
            (current_tramite.dependencia_origen &&
              current_tramite.dependencia_origen.nombre) ||
            ""
          }
      ...
      <div className="col-md-12 mb-2">
        <b>
          <i className="fas fa-clip-paper"></i> Archivos de trámite:{" "}
        </b>
        <div className="row mt-3">
          {isTramite && typeof current_tramite.files == "object"
          ...
          <Show condicion={!validateFile.includes("create")}>
            <div className="text-right ml-3">
              <DropZone
                ...
                basic={true}
                icon="fas fa-plus"
                onChange={async ({ files, name }) =>
                  await addFile({ name, file: files[0] }, current_tramite.id)
                }
                onSigned={async ({ name, file }) =>
                  await addFile({ name, file }, current_tramite.id)
                }
              />
            </div>
          </Show>
      ...
      <Show
        condicion={
          isTramite &&
          current_tramite.old_files &&
          current_tramite.old_files.length
        }
      >
        <div className="col-md-12 mb-2">
          <hr />
          <b>
            <i className="fas fa-clip-paper"></i> Archivos anidados:{" "}
          </b>
          <div className="row mt-3">
            {isTramite && typeof current_tramite.old_files == "object"
              ? current_tramite.old_files.map((f, indexF) => (
                  <div
                    className="ml-5 col-xs"
