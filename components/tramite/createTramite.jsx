...
const CreateTramite = ({
  ...
  const handleFile = async (file) => {
    ...
    if (limite >= size / 1024) setCurrentFiles([...current_files, file]);
      ...
      Swal.fire({
        icon: "warning",
        text: `El límite de 25MB fué superado (${formatBytes(size)})`,
      });
  };
  ...
  const handleDeleteFile = async ({ index }) => {
    let newFiles = [];
    await current_files.filter((f, indexF) =>
      indexF != index ? newFiles.push(f) : null
    );
            ...
            <div className="card">
              <div className="card-body">
                ...
                >
                  <Form.Field className="mb-3">
                    ...
                    <input
                      ...
                      readOnly
                      value={
                        (user && user.person && user.person.document_number) ||
                        ""
                      }
                    />
                  </Form.Field>
                  ...
                  <Form.Field className="mb-3">
                    ...
                    <input
                      ...
                      className="uppercase"
                      value={
                        (user && user.person && user.person.fullname) || ""
                      }
                    />
                  </Form.Field>
                </Show>
                ...
                <Form.Field
                  className="mb-3"
                  error={
                    errors.type_tramite_id && errors.type_tramite_id[0]
                      ? true
                      : false
                  }
                >
                  ...
                  <SelectTramiteType
                    ...
                    error={
                      errors.tramite_type_id && errors.tramite_type_id[0]
                        ? true
                        : false
                    }
                  />
                ...
                <Form.Field
                  className="mb-3"
                  error={
                    errors.document_number && errors.document_number[0]
                      ? true
                      : false
                  }
                >
                ...
                <Form.Field
                  className="mb-3"
                  error={errors.asunto && errors.asunto[0] ? true : false}
                >
                  <label>
                    Asunto <b className="text-danger">*</b>
                  </label>
                  <textarea
                    name="asunto"
                    rows="2"
                ...
                <Form.Field
                  className="mb-3"
                  error={
                    errors.observation && errors.observation[0] ? true : false
                  }
                >
                  <label>Observación</label>
                  <textarea
                  ...
                  />
                  <label>
                    {(errors.observation && errors.observation[0]) || ""}
                  </label>
                </Form.Field>

                <Form.Field
                  className="mb-3"
                  error={errors.files && errors.files[0] ? true : false}
                >
                  <label>
                    Archivos <b className="text-danger">*</b>
                  </label>
                  <DropZone
                    linkCodeQr={linkPath}
